...
export interface AgentStartEvent {
  ...
  /** Full path to the agent's JSONL transcript file — most reliable source for watching */
  ...
  return {
    SubagentStart: [{
      hooks: [async (input: HookInput): Promise<HookResult> => {
        ...
        // Extract agentId and full file path from transcript path
        // Format: ".../session-uuid/subagents/agent-ab215e8.jsonl"
        let transcriptAgentId: string | undefined;
        const transcriptFilePath = input.agent_transcript_path as string | undefined;
        if (transcriptFilePath) {
          const match = transcriptFilePath.match(/agent-([a-f0-9]+)\.jsonl$/i);
          if (match) {
